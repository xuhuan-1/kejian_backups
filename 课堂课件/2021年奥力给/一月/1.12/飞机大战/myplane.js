function MyPlane(){
    ...
    // 子弹的路径
    this.bullUrl = './images/bullet1.png';
    // 子弹移动的时间，10ms间隔子弹向上移动1px;
    this.bullSpeed = 10;
    // 装飞机的盒子
    this.container = null;
    // 子弹自身的高度
...
// 飞机X轴的位置 = 鼠标X轴的位置 - 背景距离左侧的距离 - 飞机宽度的一半
            let x = eve.clientX - this.offsetLeft - that.width/2;
// 飞机Y轴的位置 = 鼠标Y轴的位置 - 背景距离顶部的距离 - 飞机宽度的一半
            let y = eve.clientY - this.offsetTop - that.height/2;
            // 飞机盒子的位置
    ...
    moveBull(){
        ...
        this.bull.timer = setInterval(function(){
            bull.style.top = bull.offsetTop -1 + 'px';
            // 超出界限的判断
            // 当子弹距离顶端的距离< 0 - 子弹的高度时
